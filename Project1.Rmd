...
```{r prob4barplot1}
majors$CodedMajor <- as.factor(majors$CodedMajor)

majors%>%
  summarise(min = fivenumCodedMajor[1],
            Q1 = fivenumCodedMajor[2],
            med= fivenumCodedMajor[3],
            Q3= fivenumCodedMajor[4],
            max= fivenumCodedMajor[5],
            IQR = IQR(CodedMajor),
            Lower_F = Q1 - 1.5 * IQR,
            Upper_F = Q3 + 1.5 * IQR)

boxplot.stats(sort(majors$CodedMajor))
```
...
```{r prob6summaries}
  ...
  summarize(min = fivenum(before2007)[1],
            Q1 = fivenum(before2007)[2],
            med= fivenum(before2007)[3],
            Q3= fivenum(before2007)[4],
            max= fivenum(before2007)[5],
            IQR = IQR(before2007),
            Lower_F = Q1 - 1.5 * IQR,
            Upper_F = Q3 + 1.5 * IQR)
```
...
```{r prob5boxplots}
  ...
  ggplot(actress2007, aes(x = remony, y = age, fill = age)) + geom_boxplot()
